...
internal object NotiflyStorage {
    ...
    private fun createSharedPreferences(context: Context): SharedPreferences =
        context
            .getSharedPreferences(PLAIN_SHARED_PREFERENCES_NAME, Context.MODE_PRIVATE)
            .also {
    ...
    private fun getInt(
        ...
        key: String,
        defaultValue: Int,
    ): Int = getSharedPreferences(context).getInt(key, defaultValue)

    private fun getLong(
        context: Context,
        key: String,
        defaultValue: Long,
    ): Long = getSharedPreferences(context).getLong(key, defaultValue)

    private fun getFloat(
        context: Context,
        key: String,
        defaultValue: Float,
    ): Float = getSharedPreferences(context).getFloat(key, defaultValue)

    private fun getBoolean(
        context: Context,
        key: String,
        defaultValue: Boolean,
    ): Boolean = getSharedPreferences(context).getBoolean(key, defaultValue)

    private fun getString(
        context: Context,
        key: String,
        defaultValue: String,
    ): String = getSharedPreferences(context).getString(key, defaultValue) ?: defaultValue

    private fun getStringNullable(
        context: Context,
        key: String,
        defaultValue: String?,
    ): String? = getSharedPreferences(context).getString(key, defaultValue)

    fun <T> put(
        context: Context,
        item: NotiflyStorageItem<T>,
        value: T,
    ) {
